...
void AlbumManager::addPictureToAlbum()
{
	...
	m_dataAccess.addPictureToAlbumByName(getInputFromConsole("Enter album name: "), picture);
...
void AlbumManager::removePictureFromAlbum()
{
	...
	m_dataAccess.removePictureFromAlbumByName(getInputFromConsole("Enter album name: "), picture.getName());
...
void AlbumManager::removeUser()
{
	...
	const User& user = m_dataAccess.getUser(userId);
	auto yahav = m_dataAccess.getAlbumsOfUser(user);
	for (auto iter = yahav.begin(); iter != yahav.end(); iter++)
	{
		m_dataAccess.deleteAlbum(iter->getName(), userId);
	}
...
void AlbumManager::userStatistics()
{
	...
	std::cout << "user @" << userId << " Statistics:" << std::endl << "--------------------" << std::endl <<
		...
		"  + Count of Albums Owned Of User: " << m_dataAccess.countAlbumsOwnedOfUser(user) << std::endl <<
